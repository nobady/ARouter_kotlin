...
//    private var method: MethodSpec.Builder? = null
    ...
    fun buildStatement(type: Int, element: Element){
        ...
        buildStatement(sb,annotionValue,filedName,type,isCanNull)

        funcBuild.addStatement(sb.toString())
    }

    private fun buildStatement(stringBuffer: StringBuffer,parameterValueKey:String,originalValue:String,type:Int,isCanNull:Boolean){
        when(type){
            ...
            TypeKind.SERIALIZABLE.ordinal,TypeKind.OBJECT.ordinal -> {
                if (!isCanNull){
                    stringBuffer.setLength(0)
                    stringBuffer.append("t.$originalValue = ")
                    stringBuffer.append("if(t.intent.getSerializableExtra(\"$parameterValueKey\")==null) t.$originalValue else t.intent.getSerializableExtra(\"$parameterValueKey\")!!")
                }else{
                    stringBuffer.append("getSerializableExtra(\"$parameterValueKey\")")
                }
            }
            TypeKind.PARCELABLE.ordinal -> {
                if (!isCanNull){
                    stringBuffer.setLength(0)
                    stringBuffer.append("t.$originalValue = ")
                    stringBuffer.append("if(t.intent.getParcelableExtra(\"$parameterValueKey\")==null) t.$originalValue else t.intent.getParcelableExtra(\"$parameterValueKey\")!!")
                }else{
                    stringBuffer.append("getParcelableExtra(\"$parameterValueKey\")")
                }
            }
        }
    }
